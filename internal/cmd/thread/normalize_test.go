...
func TestNormalizeThread(t *testing.T) {
	apiResponse := `{
		...
		"status": "active",
		"view_url": "http://localhost:3000/theses/t1",
		"claims": [{"id": "c1"}, {"id": "c2"}],
	...
	if meta["view_url"] != "http://localhost:3000/theses/t1" {
	...
	if _, exists := meta["topic_url"]; exists {
		t.Error("topic_url should not appear in meta (field no longer emitted by Seer)")
	}
